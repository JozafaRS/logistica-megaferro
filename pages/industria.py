...
def processar_formacao_carga(arquivo):
    ...
    data_frame["Tipo de Base"] = "INDUSTRIA"
...
def page_upload():
    st.write("# Logística Megaferro - Indústria")
    col1, col2 = st.columns(2, gap='large')

    with col1:
        st.header("Enviar para o banco de dados", divider=True)

        formacao_carga = st.file_uploader("**Planilha Formação de Carga (Indústria)**", ['xlsx', 'xls'])
        portal_vendas = st.file_uploader('**Planilha Portal de Vendas (Indústria)**', ['xlsx', 'xls'])
        valores_carga = st.file_uploader('**Planilha Valores de Carga (Indústria)**', ['xlsx', 'xls'])
