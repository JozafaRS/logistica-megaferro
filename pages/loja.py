...
def page_upload():
    st.write("# Logística Megaferro - Loja")
    col1, col2 = st.columns(2, gap='large')

    with col1:
        st.header("Enviar para o banco de dados", divider=True)

        formacao_carga = st.file_uploader("**Planilha Formação de Carga (Loja)**", ['xlsx', 'xls'])
        portal_vendas = st.file_uploader('**Planilha Portal de Vendas (Loja)**', ['xlsx', 'xls'])
        valores_carga = st.file_uploader('**Planilha Valores de Carga (Loja)**', ['xlsx', 'xls'])
...
def main():
    st.set_page_config(layout='wide', page_title="Logística Megaferro - Loja")
    page_upload()
